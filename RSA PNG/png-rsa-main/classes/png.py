...
class PNG:
    ...
    def getCriticalChunks(self, verbose=False):
        critChunks = []
        try:
            with open(self.path, 'rb') as f:
                ...
                while True:
                    ...
                    if chunk_type in self.critical_chunks: 
                        critChunks.append(chunk_header + chunk_data + chunk_crc)
    ...
    def getAncillaryChunks(self, verbose=False):
        ancChunks = []
        try:
            with open(self.path, 'rb') as f:
                self.verifySignature(f)
                ...
                while True:
                    ...
                    if chunk_type not in self.critical_chunks:
                        ancChunks.append(chunk_header + chunk_data + chunk_crc)
                        if verbose:
    ...
    def getDataChunks(self, crit_chunks):
        data_chunks = []
        crit_chunks_no_data = []
        for chunk in crit_chunks:
            chunk_header = chunk[4:8]
            if chunk_header == b'IDAT':
                data_chunks.append(chunk)
            else:
                crit_chunks_no_data.append(chunk)
        return data_chunks, crit_chunks_no_data
    ...
    def showSpectrum(self):
        ...
        if (self.color_type == 0):
            ...
            plt.figure(figsize=(10, 5))
            grayscale_img = orig_img.convert('LA')
            grayscale_array = np.array(grayscale_img)[:, :, 0]

            mag_spectrum, phase_spectrum = self.getMagnitudeSpectrum(grayscale_array)

            plt.subplot(1,2,1)
            plt.imshow(mag_spectrum, cmap='gray')
            plt.title("Magnitude spectrum (Grayscale + Alpha)")

            plt.subplot(1,2,2)
            plt.imshow(phase_spectrum, cmap='gray')
            plt.title("Phase spectrum (Grayscale + Alpha)")

        elif (self.color_type == 6):
            plt.figure(figsize=(20, 10))

            rgba_array = np.array(orig_img.convert('RGBA'))
            rgb_array = np.array(rgba_array)[:, :, :3]
